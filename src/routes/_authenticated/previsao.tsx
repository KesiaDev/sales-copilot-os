...
function PrevisaoPage() {
  ...
  const necessarioPorDia = diasRestantes > 0 ? Math.max(0, meta - data.kpis.receitaMes) / diasRestantes : 0;
        ...
        {risco && (
          <div className="flex items-center gap-3 rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-destructive">
            ...
            <div className="text-sm">
              ...
              <p>Necessário <strong>{formatCurrency(necessarioPorDia)}/dia</strong> nos {diasRestantes} dias restantes para fechar em {formatCurrency(meta)}.</p>
            </div>
          </div>
        )}
          ...
          <CardContent className="space-y-6">
            <div className="grid items-center gap-6 md:grid-cols-[200px_1fr]">
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                <Stat label="Pessimista (-20%)" value={formatCurrency(projPessimista)} tone="destructive" />
                <Stat label="Realista" value={formatCurrency(projecao)} tone="primary" />
                <Stat label="Otimista (+20%)" value={formatCurrency(projOtimista)} tone="success" />
                <Stat label="Receita atual" value={formatCurrency(data.kpis.receitaMes)} />
                <Stat label="Meta" value={formatCurrency(meta)} />
                <Stat label="Gap projetado" value={formatCurrency(gap)} tone={gap > 0 ? "destructive" : "success"} />
              </div>
            </div>
            ...
            <Table>
              ...
              <TableBody>
                {velocidades.map((v: any) => {
                  ...
                  const status = pct >= 100 ? { cls: "text-success", icon: TrendingUp, label: "No ritmo" } :
                                 pct >= 80 ? { cls: "text-warning", icon: Minus, label: "Atenção" } :
                                 { cls: "text-destructive", icon: TrendingDown, label: "Em risco" };
                  return (
                    <TableRow key={v.id}>
                      <TableCell className="font-medium">{v.nome}</TableCell>
                      <TableCell className="text-right tabular-nums">{formatCurrency(v.receita)}</TableCell>
                      <TableCell className="text-right tabular-nums">{formatCurrency(v.ritmo)}</TableCell>
                      <TableCell className="text-right tabular-nums">{formatCurrency(v.projecao)}</TableCell>
                      <TableCell className="text-right tabular-nums">{formatCurrency(v.metaInd)}</TableCell>
                      <TableCell className="text-right tabular-nums">{v.diasNecessarios ?? "—"}</TableCell>
                      <TableCell className="text-right">
                        <span className={`inline-flex items-center gap-1 text-xs font-semibold ${status.cls}`}>
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "destructive" | "success" | "primary" }) {
  const cls = tone === "destructive" ? "text-destructive" : tone === "success" ? "text-success" : tone === "primary" ? "text-primary" : "";
  return (
...
function Gauge({ value }: { value: number }) {
  ...
  const color = value >= 95 ? "hsl(142 71% 45%)" : value >= 70 ? "hsl(35 95% 55%)" : "hsl(0 84% 60%)";
  return (
