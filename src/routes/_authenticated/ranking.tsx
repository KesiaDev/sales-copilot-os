...
function RankRow({ rank, nome, receita, conversao, vendas }: any) {
  return (
    <div className={`flex items-center justify-between rounded-xl border border-border/60 px-4 py-3 ${rank === 0 ? "bg-gradient-to-r from-accent/15 to-transparent" : "bg-card/50"}`}>
      <div className="flex items-center gap-4">
        <div className="w-10 text-center text-2xl">{medal(rank)}</div>
        <div>
          <div className="text-sm font-semibold">{nome}</div>
          <div className="text-xs text-muted-foreground">{vendas} vendas · {formatPercent(conversao)} conv.</div>
        </div>
      </div>
      <div className="text-right">
        <div className="text-sm font-bold tabular-nums">{formatCurrency(receita)}</div>
        <div className="text-xs text-muted-foreground">{calcScore(receita, conversao, vendas)} pts</div>
      </div>
    </div>
  );
}
...
function RankingPage() {
  ...
  const buildFromReports = (days: number) => {
    ...
    return [...map.values()].map((v) => ({ ...v, conversao: v.leads ? (v.vendas / v.leads) * 100 : 0 }))
      .sort((a, b) => b.receita - a.receita);
  };

  const daily = buildFromReports(1);
  const weekly = buildFromReports(7);
  const monthly = (data?.porVendedor ?? []).map((v: any) => ({ nome: v.nome, receita: v.receita, vendas: v.vendas, conversao: v.conversao }));

  return (
    <>
      <Topbar title="Ranking" subtitle="Performance individual com pontuação" />
          ...
          <CardContent>
            <Tabs defaultValue="monthly">
              ...
              <TabsContent value="daily" className="mt-4 space-y-2">
                {daily.length === 0 ? <p className="text-sm text-muted-foreground">Sem fechamentos hoje.</p> : daily.map((v, i) => <RankRow key={v.nome} rank={i} {...v} />)}
              </TabsContent>
              <TabsContent value="weekly" className="mt-4 space-y-2">
                {weekly.length === 0 ? <p className="text-sm text-muted-foreground">Sem dados.</p> : weekly.map((v, i) => <RankRow key={v.nome} rank={i} {...v} />)}
              </TabsContent>
              <TabsContent value="monthly" className="mt-4 space-y-2">
                {monthly.length === 0 ? <p className="text-sm text-muted-foreground">Sem dados.</p> : monthly.map((v: any, i: number) => <RankRow key={v.nome} rank={i} {...v} />)}
              </TabsContent>
            </Tabs>
          </CardContent>
        </Card>
